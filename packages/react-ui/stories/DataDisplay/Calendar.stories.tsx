...
const meta = {
	...
	args: {
		...
		onViewChange: mockedOnViewChange,
		defaultValue: [baseDate.toDate()]
	},
	beforeEach() {
		mockedOnValueChange.mockClear();
		mockedOnViewChange.mockClear();
	}
...
export const DefaultView: Story = {
	play: async ({ canvas, step, args }) => {
		const testid = args['data-testid'] || '';
		const calendar = canvas.getByTestId(testid);
...
export const ViewChangeFlow: Story = {
	play: async ({ canvas, step, args }) => {
		...
		await step('Check onViewChange event is triggered', () => {
			expect(mockedOnViewChange).toHaveBeenCalledWith(expect.objectContaining({ view: 'month' }));
		});
		...
		await step('Check onViewChange event is triggered', () => {
			expect(mockedOnViewChange).toHaveBeenCalledWith(expect.objectContaining({ view: 'year' }));
		});
		...
		await step('Check onViewChange event is triggered finding month', () => {
			expect(mockedOnViewChange).toHaveBeenCalledWith(expect.objectContaining({ view: 'month' }));
		});
		...
		await step('Check onViewChange event is triggered finding day', () => {
			expect(mockedOnViewChange).toHaveBeenCalledWith(expect.objectContaining({ view: 'day' }));
		});
	}
};
