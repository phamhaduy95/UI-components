...
export interface VirtualListProps<TData> extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
	 ...
	 */
	getItemKey?: (index: number) => import('react').Key;
	...
	/**
	 * Render prop for each individual row (or column in horizontal mode).
	 * @param props - Contains the zero-based position and the data object.
	 */
	children: (props: { index: number; itemData: TData }) => ReactNode;
	 ...
	 * @param details direction - `'forward'` when scrolling toward the end, `'backward'` when scrolling toward the start. offsetInPixel - The current scroll offset.
	 */
	onScrolling?: (details: { direction: ScrollDirection; offsetInPixel: number }) => void;
	 ...
	 * @param range startIndex - Zero-based index of the first currently visible item. endIndex - Zero-based index of the last currently visible item.
	 */
...
export interface VirtualListPublicInstance {
	 ...
	 * @param options - Optional scroll behaviour options.
	 */
